 ...
 <hr style="clear:both;">
          <div class="table-responsive">
            ...
            <table class="table  table-bordered">
              ...
              <tbody id="userData">
                  <c:forEach items="${requestScope.employeeMap.employees}" var="emp" varStatus="status">
					  <tr >
						  ...
						  <th>${emp.lastName}</th>
						  <th>${emp.email}</th>
						  <th width="100">操作${requestScope.employeeMap.pageNo}/${requestScope.employeeMap.totalPage}</th>
					  </tr>
				  </c:forEach>
              </tbody>
			  ...
			  <tfoot>
			     <tr >
				     <td colspan="6" align="center">
						<ul class="pagination">
							<c:set var="pageNoNow" value="${requestScope.employeeMap.pageNo}"></c:set>
							<c:set var="totalPage" value="${requestScope.employeeMap.totalPage}"></c:set>
							<c:if test="${pageNoNow > 1}">
								<li><a href="?pageNo=${pageNoNow - 1}">上一页</a></li>
							</c:if>
							<c:forEach begin="1" end="${totalPage}" varStatus="totalPage">
								<c:if test="${pageNoNow == totalPage.count}">
									<li class="active"><a href="javascript:;" >${totalPage.count}</a></li>
								</c:if>
								<c:if test="${pageNoNow != totalPage.count}">
									<li><a href="?pageNo=${totalPage.count}">${totalPage.count}</a></li>
								</c:if>
							</c:forEach>
							<c:if test="${pageNoNow != totalPage}">
								<li><a href="?pageNo=${pageNoNow + 1}">下一页</a></li>
							</c:if>
						</ul>
					 </td>
				 </tr>
